#include <iostream>
using namespace std;
int main() {
  /*
   * Buchtitel
   */
  std::cout << std::endl << "Die Maerchen von TESxx" << std::endl << std::endl;
  /*
   * Erstes Kapitel
   */
  std::cout << std::endl
            << "Kapitel 1: Der verlorene Schatz" << std::endl
            << std::endl;

  std::cout << std::endl
            << "Haini hat den Schatz verloren." << std::endl
            << std::endl;

  std::cout
      << "Vor langer Zeit, in einem fernen Königreich, gab es einen König"
         "und eine Königin, die sich sehnlichst wünschten, keine König und"
         "Königin mehr zu sein."
      << std::endl
      << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                    // unter diesem Satz

  /*
   * Zweites Kapitel
   */
  std::cout << std::endl
            << "Kapitel 2: Die sieben Zwerge und der Drachen" << std::endl
            << std::endl;

  std::cout
      << "Vor langer langer Zeit lebte einst ein kleines Maedchen, das alle "
         "Rotkaeppchen nannten, denn es trug nie ein feuerrotes Maentelchen."
      << std::endl
      << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst unter
                    // diesem Satz

  /*
   * Drittes Kapitel
   */
  std::cout << std::endl
            << "Kapitel 3: Das Traumfresserchen" << std::endl
            << std::endl;
  std::cout << "Es war einmal ein Koenigreich, in dem war seit einem Jahr die "
               "Sonne nicht mehr aufgegangen."
            << std::endl
            << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                          // unter diesem Satz
    /*
     * Viertes Kapitel
     */
    std::cout << std::endl
              << "Kapitel 4: Der Prinz und der Frosch" << std::endl
              << std::endl;
    std::cout << "Vor langer Zeit gab es einmal einen magischen See, der war so "
                 "besonders, dass er nur von denjenigen gefunden werden konnte, "
                 "von denen der See sich finden lassen wollte."
              << std::endl
              << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                            // unter diesem Satz
   std::cout << "Jeff ist ein ganz normaler Frosch" << std::endl

  /*
   * Viertes Kapitel
   */
  std::cout << std::endl
            << "Kapitel 4: Der Prinz und der Frosch" << std::endl
            << std::endl;
  std::cout << "Vor langer Zeit gab es einmal einen magischen See, der war so "
               "besonders, dass er nur von denjenigen gefunden werden konnte, "
               "von denen der See sich finden lassen wollte."
            << std::endl
            << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                          // unter diesem Satz

  /*
   * Fünftes Kapitel
   */
  std::cout << std::endl
            << "Kapitel 5: Die Maus, der Vogel und die Bratwurst" << std::endl
            << std::endl;

  std::cout
      << "Es waren einmal eine Maus, ein Vogel und ein Bratwurst, die lebten "
         "zusammen in einer kleinen Huette am Rande eines verwunschenen "
         "Schrottplatzes."
      << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                    // unter diesem Satz
    std::cout
        << "Es waren einmal eine Maus, ein Vogel und ein Bratwurst, die lebten "
           "zusammen in einer kleinen Huette am Rande eines verwunschenen Schrottplatzes."
        << std::endl; // erster Satz, neue Sätze in diesem Kapitel erst
                      // unter diesem Satz
   
     std::cout
        << "Sie waren beste Freunde und verbrachten ihre Tage damit, Abenteuer zu erleben und die Welt um sie herum zu erkunden."
        << std::endl;
}